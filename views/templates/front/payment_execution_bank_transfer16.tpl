...
{capture name=path}{l s='Bank Transfer' mod='payxpert'}{/capture}
...
{else}
  {if $smarty.const._PS_VERSION_ < 1.6}<h3>{l s='Bank Transfer' mod='payxpert'}</h3>{/if}
    <form action="{$this_link|escape:'html'}" method="post">
       {if $smarty.const._PS_VERSION_ < 1.6}
         <p>
       {else}
          <div class="box cheque-box">
          <h3 class="page-subheading">{l s='Bank Transfer' mod='payxpert'}</h3>
          <p class="cheque-indent">
           <strong class="dark">
        {/if}
            <img src="{$this_path|escape:'htmlall':'UTF-8'}views/img/payment-types/{$payment_logo|escape:'htmlall':'UTF-8'}.png" alt="{l s='Bank Transfer' mod='payxpert'}" style="float:left; margin: 0px 10px 5px 0px;" />
            {l s='You have chosen to pay by Bank Transfer.' mod='payxpert'}  {l s='Here is a short summary of your order:' mod='payxpert'}
