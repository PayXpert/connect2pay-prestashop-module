...
{if $status == 'ok'}
	<p>{l s='Your order has been completed.' mod='payxpert'}
		...
		<br /><br />{l s='For any questions or for further information, please contact our' mod='payxpert'} <a href="{$this_link_contact|escape:'htmlall':'UTF-8'}">{l s='customer support' mod='payxpert'}</a>.
	</p>
{else}
	{if $status == 'pending'}
	...
	{else}
		<p class="warning">
			...
			<a href="{$this_link_contact|escape:'htmlall':'UTF-8'}">{l s='customer support' mod='payxpert'}</a>.
		</p>
	{/if}
{/if}